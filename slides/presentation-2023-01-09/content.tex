...
\begin{frame}{Block Gibbs Sampler}
    ...
    % \begin{itemize}
    %     \item Precision Matrix: $\small P(\bm{K} \mid \bm{Y},G,\bm{z}) \propto P(\bm{Y} \mid \bm{K})P(\bm{K} \mid {G})$ 

    %     \item Graph: $\small P(G \mid \bm{Y},\bm{K},\bm{z}) \propto P(\bm{Y} \mid \bm{K})P(\bm{K} \mid {G})P(G \mid \bm{z})$ 

    %     \item Random Partition: $\small P(\bm{z} \mid \bm{Y},\bm{K},G) \propto P(\bm{Y} \mid \bm{K})P(\bm{K} \mid {G})P(G \mid \bm{z})P(\bm{z}) \propto P(G \mid \bm{z})P(\bm{z}) $
    % \end{itemize}
    ...
    \begin{table}[tb]
        ...
        \begin{tabular}{ll}
        \toprule
        Precision Matrix & $\small P(\bm{K} \mid \bm{Y},G,\bm{z}) \propto P(\bm{Y} \mid \bm{K})P(\bm{K} \mid {G})$ \\
        \hline
        Graph & $\small P(G \mid \bm{Y},\bm{K},\bm{z}) \propto P(\bm{Y} \mid \bm{K})P(\bm{K} \mid {G})P(G \mid \bm{z})$ \\
        \hline
...
\[
    ...
    =
    \frac{P(\text{choose split})}{P(\text{choose merge})}
    \cdot 
    \frac{P(\text{split at node $i$})}{P(\text{merge at node $i$})}
    =
    \frac{\alpha_{\text{split}}}{1-\alpha_{\text{split}}}
    \cdot
...
Next things to do:
    \begin{itemize}
        ...
        \item \textcolor{red}{TODO what else? G.C.}
    \end{itemize}
...
The likelihood ratio, after simplifying common factors, is:

{
\footnotesize
\begin{align*}
&\frac{f_G(G \mid \rho^\prime)}{f_G^{\prime}(G \mid \rho)}=\left(\frac{1}{B(\alpha, \beta)}\right)^{M+1} \times \\
