...
\centering
\begin{align*}
    \mathbf{y_{1}}, \ldots, \mathbf{y_{n}} \mid \mathbf{K} &\iid N_\mathbf{{p}}(0,\mathbf{K}), \mathbf{K}=\Sigma^{-1}\\  %%%%add bold
...
\end{columns}
\centering
How to introduce an appropriate prior for \alert{partition generation} x(4) and a model for the \alert{edge generation} conditionally on z (3)? 
\end{frame}


%%TO DO @Teo 
% Add numeration of formulas from 1 to 4
%Add the tap for the block underligning point 3 and 4
%Add bold sigma
%Add the image
