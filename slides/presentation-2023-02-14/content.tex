...
We performed posterior analysis using the following metrics and plots:
    \begin{enumerate}
        \item The partition $\bm{\rho}$ is estimated by solving a minimization problem using decision theory restricted to the space of visited partitions using the \alert{VI} loss function
        \item Mean acceptance rate
        \item The \alert{Rand index} to measure the similarity between partitions
        \item The adjacency matrix is estimated by taking the \alert{p-links} matrix (probability that a link is included in the graph) and choosing a threshold value $s$ using BFDR
        \item Plot of the estimated \alert{precision matrix} vs generating precision matrix
        \item \alert{Kullback-Leibler} distance comparing the precision matrices
...
The traceplots of $\theta$ and $\sigma$ are similar throughout all the simulations.
