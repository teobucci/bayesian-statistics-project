...
\begin{frame}{Block Gibbs Sampler}
    ...
    \begin{enumerate}
        ...
        $G$ and $\bm{K}$ - given $\bm{z}$ - are sampled using a modified version of a Birth-and-Death chain (\cite{mohammadiBayesianStructureLearning2015a}), changing one link at a time. The modified Birth and Death rates take into account the dependency on the random partition.
...
We ran a total of 42 simulations, varying different hyperparameters. 
...
We performed posterior analysis using the following metrics and plots:
    \begin{enumerate}
        \item Estimated \alert{p-links} matrix vs generating adjacency matrix
...
\fg{0.82}{../../output/data/simulation_02/comparison_G_true_est.pdf}
\end{minipage}
\hfill
\begin{minipage}[c]{0.49\linewidth}
\fg{0.82}{../../output/data/simulation_02/comparison_K_true_est.pdf}
\end{minipage}%
\end{figure}
\end{frame}
...
\end{tabular}}
\end{table}
\fg{0.65}{../../output/data/simulation_02/indexes.pdf}
\end{frame}
...
Varying Beta\_sig2 does not yield significant changes in posterior analysis.
...
\begin{figure}
\begin{minipage}[c]{0.49\linewidth}
\fg{1.01}{../../output/data/simulation_22/comparison_G_true_est.pdf}
\end{minipage}
\hfill
\begin{minipage}[c]{0.49\linewidth}
\fg{1.01}{../../output/data/simulation_22/comparison_K_true_est.pdf}
\end{minipage}%
\end{figure}
\end{frame}
...
\fg{0.6}{../../output/data/simulation_22/indexes.pdf}
...
As $n$ diminishes and becomes comparable to $p$, we notice that the KL-distance plateus on progressively higher values, and \texttt{VI} and \texttt{RI} behave accordingly.
...
\begin{figure}
\begin{minipage}[c]{0.49\linewidth}
\fg{0.82}{../../output/data/simulation_41/comparison_G_true_est.pdf}
\end{minipage}
\hfill
\begin{minipage}[c]{0.49\linewidth}
\fg{0.82}{../../output/data/simulation_41/comparison_K_true_est.pdf}
\end{minipage}%
\end{figure}
...
\fg{0.55}{../../output/data/simulation_41/graph.pdf}
...
\fg{0.6}{../../output/data/simulation_41/indexes.pdf}
...
$\theta$ and $\sigma$ are updated according to \textcite[835-836]{martinezNonparametricChangePoint2014}.

Their traceplots are similar throughout all the simulations.
A good mixing of the chain is to be appreciated:

\fg{0.7}{../../output/data/simulation_41/theta_sigma_prior.pdf}
