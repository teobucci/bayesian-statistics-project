...
\end{table}
    ...
    \begin{enumerate}
        ...
        \item \alert{Kullback-Leibler} (\alert{KL}) distance to compare generating and estimated precision matrixes
    \end{enumerate}

        \pause

    \begin{itemize}
        \item Adjacency matrix estimated by taking the \alert{p-links} matrix (probability that a link is included in the graph) and choosing a threshold value $s$ using BFDR
...
Changing the \texttt{variance of the beta prior} does not yield significant changes in posterior analysis.
