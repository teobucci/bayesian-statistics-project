...
We ran a total of 42 simulations, varying different parameters for our model. 
The simulations are subdivided in groups, depending on which parameter was tuned:
    \begin{enumerate}
        ...
        \item Starting partition's alert{cluster numerosities}
...
We performed posterior analysis using the following metrics and plots:
    \begin{enumerate}
        \item Estimated \alert{p-links} matrix vs true adjacency matrix
        \item Estimated \alert{precision matrix} vs true precision matrix
        \item \alert{Changepoint frequency} distribution (estimated vs true partition)
        \item Random index traceplot
        \item \alert{Kullback-Leibler} distance curve
        \item $\theta$ and $\sigma$ prior
...
\fg{0.82}{Images/group_1/comparison_G_true_est.pdf}
\end{minipage}
\hfill
\begin{minipage}[c]{0.49\linewidth}
\fg{0.82}{Images/group_1/comparison_K_true_est.pdf}
\end{minipage}%
\end{figure}
\end{frame}
...
\end{tabular}}
\end{table}
\fg{0.65}{Images/group_1/graph.pdf}
\end{frame}
...
\begin{frame}{Group 2}
\fg{0.8}{Images/kl_dist_comparison_beta_sig2-1.pdf}
...
The traceplots for $\theta$ and $\sigma$ are similar throughout all the simulations.
We can notice a good mixing of the chain:
