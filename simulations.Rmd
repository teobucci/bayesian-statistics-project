...
barplot(
    freq$freq,
    names = seq_along(freq$freq),
    border = "NA",
    space = 0,
    yaxt = "n"
)
abline(v=indexes-0.5, col='red')
...
plot(x =  1:length(num_clusters), y = num_clusters, type = "n", xlab='Iterations', ylab='Number of groups', main='Number of groups - Traceplot')
lines(x =  1:length(num_clusters), y = num_clusters)

abline(h=length(z_to_rho(z_true)), col='red', lwd=4)
```
...
#index of the first iteration (after the burnin)
...
plot(x =  1:length(kl_dist), y = kl_dist, type = "n", xlab='Iterations', ylab='K-L distance', main=paste('Kullback–Leibler distance - Last value:',last))
