...
library("FGM")
library("gmp")
library("mcclust") # masks binder from salso
...
KL_dist(Omega_true,res$K_final)
final_graph <- matrix(0,p,p)
final_graph[which(res$graph>0.5)] = 1

for(soglia in seq(0.5,1,0.01)){
    BFDR_graph_num = 0
    BFDR_graph_den = 0
    id_graph = matrix(0,p,p)
    id_graph[which(res$graph>soglia)] = 1
    for(k in 1:p){
        for(j in 1:k){
        BFDR_graph_num <- BFDR_graph_num + (1-res$graph[k,j])*(id_graph[k,j])
...
g <- graph.adjacency(final_graph, mode = "undirected")
# Plot the graph
plot(g)
