...
#grid = read.table('grid.csv',sep=',',header = TRUE)
...
for(i in seq_len(length(grid))){
    ...
    rho_true = grid[[i]] # c(8,4,8,5)

    # Set seed for data generation
    seed_data_gen = 27121996 #grid[i,"seed_data_gen"]
    ...
    log_print("Execution time:", console = FALSE)
    log_print(Sys.time() - start_time, console = FALSE)
    log_close()
