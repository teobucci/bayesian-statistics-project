...
Given this model, our goal is to propose a prior distribution of $\bm{z}$ which account for the ordering constraint on the nodes, assigning a probability law over the space of admissible partitions.
...
We say that $\bm{\rho}$ is admissible if, for each $i<j$, each element of $C_i$ is strictly smaller than each element of $C_j$. 
