...
\subsection{Performance indexes: Kullback-Leibler}
...
\subsection{Performance indexes: Rand index}
...
Simulations were divided into groups, depending on the parameter tuned, and posterior analysis was carried out on each simulation using the same plots and metrics:
 \begin{enumerate}
        \item $\bm{\rho}$ estimated by solving a minimization problem on the space of \emph{visited} partitions using Variation of Information (VI) loss function
        \item Mean acceptance rate
        \item Rand index (RI) used to measure the similarity between partitions
        \item Kullback-Leibler (KL) distance to compare generating and estimated precision matrices
\end{enumerate}
...
For following groups, only relevant plots highlighting differences are included.
...
Moreover, most of the clusters are indeed recognised as shown in the barplot below: the problem lies in the criteria for selecting the clusters.
...
As $n$ diminishes and becomes comparable to $p$, the KL distance plateaus on progressively higher values, and VI and RI behave accordingly.
...
The data generator for this simulation is a noised block structure.
