...
The proposal distribution $Q(\bm{z},\bm{z}')$ is based on property that the nodes are ordered. % TODO non bellissima
% TODO potremmo aggiungere tutti i casi più generali scritti nel file pdf di colombi
...
\end{align*}
It's enough to compute it both with the current partition $\bm{z}$ and with the proposed one $\bm{z}'$ and compute the ratio simplifying common factors. For further details about the resulting expression see Appendix TODO ref
...
after simplifying common factors, it is computed as:
TODO chi sono C'?
\begin{align*}
