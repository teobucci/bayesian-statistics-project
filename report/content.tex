...
\end{enumerate}
As mentioned in \textbf{Section 2}, we adopt three possible ways to express the partition depending on the aspect is most convenient and we wish to highlight, which we recapitulate in the list below. In all the representations, we will denote by $M$ the number of groups the nodes have been partitioned into, with $C_{1}, \dotsc, C_{M}$ the groups, with $n_{j} = |C_{j}|$ and with $p$ the number of nodes.
\begin{itemize}
