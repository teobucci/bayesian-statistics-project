...
Sampling from the partition
\begin{align*}
G\mid \mathbf{z} &\sim f_{G}( G\mid \mathbf{z})\\
\rho &\sim \mathcal{L}( \rho )
\end{align*}
We adopt three possible ways to express the partition
\begin{itemize}
...
\item $\mathbf{z} =( z_{1} ,\dotsc ,z_{p})$ is a $p$-dimensional vector of memberships, $z_{j} =m$ if node $j$ belongs to group $C_{j}$
\item $\mathbf{r} =( r_{1} ,\dotsc ,r_{p-1})$ is a $( p-1)$-dimensional vector which tells us if a node is the right end of a group. The last node is, by definition, always the end of a group, at most its own group, thus technically can always be thought of as $1$.
\end{itemize}

Example
\begin{equation*}
\rho =( 1,3,2,2) \ \ \iff \ \ \mathbf{z} =( 1,2,2,2,3,3,4,4) \ \ \iff \ \ \mathbf{r} =( 1,0,0,1,0,1,0)
\end{equation*}
...
\begin{equation*}
\mathbf{a}^{( t)} =( a_{1}^{( t)} ,\dotsc ,a_{p-1}^{( t)}) \quad \mathbf{d}^{( t)} =( d_{1}^{( t)} ,\dotsc ,d_{p-1}^{( t)})
\end{equation*}
where
\begin{itemize}
...
These two densities are not normalized.

\section{Sampling strategy}
We proposed a Block Gibbs-Sampler strategy divided into two main chunks: 
\begin{enumerate}
    \item sampling of the graph $G$ and the precision matrix $\bm{K}$
...
Given the membership vector $\bm{z}$, a Birth-and-Death approach is used to sample the graph, as suggested in the work of Mohammadi and Wit. (TODO : add citation).
The Birth-and-Death algorithm decides at every iteration of the Gibbs Sampler whether to add a new edge to the graph (birth) or delete an already existing one (death).
For this purpose, we modified the R package \textit{BDgraph} to take into account the dependency from membership vector $\bm{z}$, updating the target distribution and the birth and death rates as follows: (TODO : dovremmo aggiungere i numeri alle formule?)
\[
...
For the second part of our sampling strategy, the random partition is sampled conditionally on the graph G using the conditional distribution:

$$ \small P(\bm{z} \mid \bm{Y},\bm{K},G) \propto P(\bm{Y} \mid \bm{K})P(\bm{K} \mid {G})P(G \mid \bm{z})P(\bm{z}) \propto P(G \mid \bm{z})P(\bm{z}) $$
...
The $\alpha_{\text{target}}$ is: $$f(\rho \mid G) \approx f_G(G \mid \rho) f_{\rho}(\rho)$$
...
To compute the proposal ratio we first need to introduce two $(p-1)$ dimensional vectors of weights: $\bm{a}^{(t)}$ and $\bm{d}^{(t)}$. They are unnormalized discrete densities that  are used to choose the node to perform the split or the merge. 
...
For example, supposing a splitting move, the probability to draw node $i$ for the split is proportional to its weight $\bm{a}_{i}^{(t)}$.
 ...
 We then introduce: $$a^{\star} = \sum_{j\in \{\text{admissible split nodes}\}}{a_{j}^{(t)}}$$  $$d^{\star} = \sum_{j\in \{\text{admissible merge nodes}\}}{d_{j}^{(t)}}$$
...
As a prior, we use an EPPF induced by the two-parameter Poisson-Dirichlet process (Pitman-Yor process) from \cite{martinezNonparametricChangePoint2014}.

Let $M$ and $p$ be the number of groups and nodes, respectively, and $n_{j},j=1,\ldots,M$ the cardinalities of the groups, with $\theta$ and $\sigma$ parameters.
...
The shuffle move is then accepted using Metropolis-Hastings where the acceptance probability is calculated as: (TODO: controllare formule)
 \[
   ...
   \bigg\{1,
   \underbrace{\frac{P(G\mid \bm{z'})}{P(G\mid \bm{z})}}_{\substack{\text{Likelihood}\\\text{ratio}}}
...
Where the likelihood ratio, since  the number of groups M, in this case, doesn't change, is computed as: (TODO : controllare formule)
 \[
   P(G\mid \bm{z}) = \prod_{l=1}^M \prod_{l=m}^M \frac{B(\alpha+S_{uv},\beta+S^{\star}_{uv})}{B(\alpha,\beta)} = \left(\frac{1}{B(\alpha,\beta)}\right)^\frac{M(M+1)}{2}\prod_{l=1}^M \prod_{l=m}^M B(\alpha+S_{uv},\beta+S^{\star}_{uv})
\]

\subsubsection{Adaptive step}

The adaptive step consists of updating the two weights vectors $\bm{a}^{(t)}$ (in case of a split move) and $\bm{d}^{(t)}$ (in case of a merge move) at each iteration $t$ as in \cite{bensonAdaptiveMCMCMultiple2018} (TODO : right citation?) using the following scheme:
    \begin{itemize}
...
$$\hat{p}_{j k}=\frac{\sum_{t=1}^T \mathbbm{1}_{\left((j, k) \in E_t\right)} w\left(\boldsymbol{G}_t\right)}{\sum_{t=1}^T w\left(\boldsymbol{G}_t\right)}$$

where $\mathbbm{1}_{\left((j, k) \in E_t\right)}$ is the indicator function for the existence of the edge linking node j and node k at iteration t and $\left(\boldsymbol{G}_t\right)$ is the graph weight at iteration t.
...
\begin{itemize}
    \item  $s$ = 0.5, similar to the median probability model proposed by Barbieri and Berger (2004)
    \item the Bayesian False Discovery rate.
